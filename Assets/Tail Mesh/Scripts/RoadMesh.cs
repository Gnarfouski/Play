...
#region structs and enums
    ...
    public void OnDrawGizmos()
    {
        ...
        if (myVertices != null)
        {
            ...
            {
                for(int j = 0;j < myVertices[i].Count; j++)
                {
                    Vector3 pos = myVertices[i][j].coordinates;
                    Gizmos.DrawSphere(transform.TransformPoint(pos), 0.3f);

                    string res = i + "a" + j + "a" + myVertices[i][j].globalPosition;

                    TextGizmo.Draw(transform.TransformPoint(pos), res);
                }
            }
        }
        //*/
        ...
        if (m_newMesh != null)
        {
            for (int i = 0; i < m_newMesh.vertexCount / 4; i++)
            {
                    Vector3 pos = m_newMesh.vertices[i];
                    Gizmos.DrawSphere(transform.TransformPoint(pos), 0.3f);
                    TextGizmo.Draw(transform.TransformPoint(pos), i.ToString());
            }
        }
        //*/
    }
    ...
    public void GenerateMesh()
    {
        ...
        RoadMeshGeneration G = new RoadMeshGeneration();

        Mesh newMesh = G.GenerateMesh(m_Points, roadwidths, m_SegmentSize);
